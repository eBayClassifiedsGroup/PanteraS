...
curl -X DELETE -H "Content-Type: application/json" http://${IP}:8080/v2/apps/webapp1?force=true >/dev/null 2>&1
curl -X DELETE -H "Content-Type: application/json" http://${IP}:8080/v2/apps/webapp2?force=true >/dev/null 2>&1
sleep 1
